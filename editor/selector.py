...
class ArticleSelector:
    ...
    def __init__(
        ...
        print(f"[EDITOR] Initialized with model: {model}")
    ...
    def _format_candidates_for_prompt(
        ...
        for article in candidates:
            simplified.append({
                "id": article.get("id", ""),
                "title": article.get("title", "")[:150],  # Truncate long titles
                "source_id": article.get("source_id", ""),
                "source_name": article.get("source_name", ""),
                "link": article.get("link", ""),
                "published": article.get("published", ""),
                "ai_summary": article.get("ai_summary", "")[:300],  # Truncate summaries
                "tags": article.get("tags", [])[:5],  # Limit tags
                "has_image": article.get("image", {}).get("has_image", False),
            })
        
        candidates_json = json.dumps(simplified, indent=2, ensure_ascii=False)
